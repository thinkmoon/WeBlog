...
class towxml {
	constructor(option) {
		...
		_ts.m.md.renderer.rules.emoji = function(token, idx) {
			return '<g-emoji class="h2w__emoji h2w__emoji--' + token[idx].markup + '">' + token[idx].content +
				'</g-emoji>';
		};
	}
		...
		if (app) {
			(eachData = (data) => {
				...
				if (data.type === 'audio') {
					...
					if (data._e.attr.src.indexOf('//') === 0) {
						data._e.attr.src = `https:${data._e.attr.src}`;
					}

					if (data._e.attr.poster.indexOf('//') === 0) {
						data.attr.poster = `https:${data._e.attr.poster}`;
					}
