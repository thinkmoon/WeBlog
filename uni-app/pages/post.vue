<template>
  <block>
    ...
    <view
      class="like-area solid-top bg-white tm-every-center flex-direction"
      v-if="!isLoading"
    >
      <view
        class="like-btn text-xxl line-base tm-every-center padding border"
        @click="like"
        v-if="!isLike"
        >赞</view
      >
      <view
        class="like-btn text-xxl line-red tm-every-center padding border"
        v-else
        @click="reward"
        >赏</view
      >
      ...
      <view class="comment-area bg-white margin-top padding">
        <view class="action" v-if="likeUsers != null">
          ...
          <text class="text-grey text-sm"
            >{{ likeUsers[0].nickName }}等{{ likeUsers.length }}人觉得很赞</text
          >
        </view>
        <!-- #ifndef MP-QQ -->
        <textarea
